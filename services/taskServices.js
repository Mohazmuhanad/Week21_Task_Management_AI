...
import { generateTaskWithAI } from "./aiService.js";
...
// Generate task from AI prompt
export async function generateTaskFromPrompt(prompt, userId) {
  try {
    // Use the AI service to generate task data
    const taskData = await generateTaskWithAI(prompt);
    ...
    return {
      success: true,
      message: "Task generated successfully using AI",
      data: createdTask,
      aiPrompt: prompt,
    };
  } catch (error) {
    throw new Error(`Error generating task with AI: ${error.message}`);
  }
}
